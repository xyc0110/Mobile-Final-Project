...
from changedetection import SitupDetection
...
def run(
    ...
    cd = SitupDetection(up_threshold=400, down_threshold=280, video_id=Path(source).stem)
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in det:
                    if int(cls) == 0:  # person
                        ...
                        bbox = (x1, y1, x2, y2)
                        annotator.box_label(xyxy, f"person Count:{cd.count}", color=colors(0, True))
                        break
